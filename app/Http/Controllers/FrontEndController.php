<?php

namespace App\Http\Controllers;

use Illuminate\Http\Request;

class FrontEndController extends Controller
{
    public function index(){
        return view('index');
    }
    public function aboutTeam(){
        return view('about.team');
    }
    public function aboutHistory(){
        return view('about.history');
    }
    public function aboutExecutive(){
        return view('about.executive');
    }
    public function contact(){
        return view('contact');
    }    
   
    public function network(){
        return view('our-network');
    }
    public function research(){
        return view('research.index');
    }
   
    public function research1(){
        return view('research.research1');
    }
    public function research2(){
        return view('research.research2');
    }
    public function research3(){
        return view('research.research3');
    }
    public function research4(){
        return view('research.research4');
    }
    public function research5(){
        return view('research.research5');
    }
    public function research6(){
        return view('research.research6');
    }
    public function research7(){
        return view('research.research7');
    }
    public function research8(){
        return view('research.research8');
    }
    public function otherResearch(){
        return view('other-research');
    }
    public function advocacy(){
        return view('advocacy.index');
    }
    public function advocacy1(){
        return view('advocacy.advocacy1');
    }
    public function advocacy2(){
        return view('advocacy.advocacy2');
    }
    public function advocacy3(){
        return view('advocacy..advocacy3');
    }
    public function capacityBuilding(){
        return view('capacity_building.index');
    }
    public function capacityBuilding1(){
        return view('capacity_building.capacity-building1');
    }
    public function capacityBuilding2(){
        return view('capacity_building.capacity-building2');
    }
     public function communication(){
        return view('communication.index');
    }
    public function communication1(){
        return view('communication.communication1');
    }
    public function communication2(){
        return view('communication.communication2');
    }
    public function communication3(){
        return view('communication.communication3');
    }
    public function communication4(){
        return view('communication.communication4');
    }
    public function roadSefety(){
        return view('road_sefety.index');
    }
    public function roadSefety1(){
        return view('road_sefety.road-safety1');
    }   
    public function migration(){
        return view('migration.index');
    }
    public function migration1(){
        return view('migration.migration1');
    }
    public function migration2(){
        return view('migration.migration2');
    }
    public function publicHealth(){
        return view('public_health.index');
    }
    public function publicHealth1(){
        return view('public_health.public-health1');
    }
    public function publicHealth2(){
        return view('public_health.public-health2');
    }
    public function publicHealth3(){
        return view('public_health.public-health3');
    }
    


    public function socialMedia(){
        return view('social-media');
    }
    public function mainstreamMedia(){
        return view('mainstream_media.index');
    }
    public function mainstreamMedia1(){
        return view('mainstream_media.mainstream1');
    }
    public function mainstreamMedia2(){
        return view('mainstream_media.mainstream2');
    }
    public function mainstreamMedia3(){
        return view('mainstream_media.mainstream3');
    }
    public function mainstreamMedia4(){
        return view('mainstream_media.mainstream4');
    }
    public function mainstreamMedia5(){
        return view('mainstream_media.mainstream5');
    }
    public function mainstreamMedia6(){
        return view('mainstream_media.mainstream6');
    }
    public function mainstreamMedia7(){
        return view('mainstream_media.mainstream7');
    }
    public function mainstreamMedia8(){
        return view('mainstream_media.mainstream8');
    }
    public function electronicMedia(){
        return view('electronic_media.index');
    }
    public function electronicMedia1(){
        return view('electronic_media.electronic-media1');
    }
    public function electronicMedia2(){
        return view('electronic_media.electronic-media2');
    }
    public function electronicMedia3(){
        return view('electronic_media.electronic-media3');
    }
    public function electronicMedia4(){
        return view('electronic_media.electronic-media4');
    }
    public function recentActivities(){
        return view('activity.index');
    }
    public function activity1(){
        return view('activity.activity1');
    }
    public function activity2(){
        return view('activity.activity2');
    }
    public function activity3(){
        return view('activity.activity3');
    }
    public function activity4(){
        return view('activity.activity4');
    }
    public function activity5(){
        return view('activity.activity5');
    }
    public function activity6(){
        return view('activity.activity6');
    }
    public function activity7(){
        return view('activity.activity7');
    }
    public function activity8(){
        return view('activity.activity8');
    }
    public function achivement(){
        return view('achivement.index');
    }
    public function achivement1(){
        return view('achivement.achivement1');
    }
    public function achivement2(){
        return view('achivement.achivement2');
    }
    public function publication(){
        return view('publication.index');
    }
    public function publication1(){
        return view('publication.publication1');
    }
    public function publication2(){
        return view('publication.publication2');
    }
    public function publication3(){
        return view('publication.publication3');
    }
    public function publication4(){
        return view('publication.publication4');
    }
    public function publication5(){
        return view('publication.publication5');
    }
    public function publication6(){
        return view('publication.publication6');
    }
    public function publication7(){
        return view('publication.publication7');
    }
    public function publication8(){
        return view('publication.publication8');
    }

    public function climateChange(){
        return view('climate_change.index');
    }
    public function climateChange1(){
        return view('climate_change.climate-change1');
    }
    public function climateChange2(){
        return view('climate_change.climate-change2');
    }
    public function climateChange3(){
        return view('climate_change.climate-change3');
    }
    public function climateChange4(){
        return view('climate_change.climate-change4');
    }
    public function consultancy(){
        return view('consultancy.index');
    }
   

    
}
